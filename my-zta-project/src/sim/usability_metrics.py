...
class SUSCalculator:
    ...
    def _simulate_responses(self, results: List[TaskResult]) -> List[int]:
        ...
        responses.append(round(6 - (avg_friction + 1)))
        ...
        responses.append(round(6 - (friction_variance + 1)))
        ...
        early_results = results[:len(results)//3]
        early_success = sum(1 for r in early_results if r.success) / len(early_results)
        responses.append(round(6 - (early_success * 5)))
